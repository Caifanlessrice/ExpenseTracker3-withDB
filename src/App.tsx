...
const CATEGORY_COLORS: Record<Category, string> = {
  ...
  Bills: "#FFA07A", "Skincare & Necessities": "#C084FC", Others: "#98D8C8"
};
const CATEGORY_ICONS: Record<Category, string> = {
  Food: "🍔", Transport: "🚌", Shopping: "🛍️",
  Bills: "📄", "Skincare & Necessities": "🧴", Others: "📦"
};
 ...
 * Falls back to SAMPLE_DATA on first launch (no data saved yet).
 * Falls back to in-memory state if storage is unavailable.
 */
async function loadTransactions(): Promise<Transaction[]> {
  try {
    // Try window.storage first (Cloudflare Pages persistent KV)
    if (typeof window !== "undefined" && window.storage) {
      const result = await window.storage.get(STORAGE_KEY);
      if (result && result.value) {
        return JSON.parse(result.value) as Transaction[];
      }
      // First launch — seed with sample data and persist it
      await window.storage.set(STORAGE_KEY, JSON.stringify(SAMPLE_DATA));
      return SAMPLE_DATA;
    }
  } catch {
    // storage key doesn't exist yet or other error — fall through
  }
 ...
 */
async function saveTransactions(data: Transaction[]): Promise<void> {
  ...
  try {
    ...
    // storage completely unavailable — data only lives in memory this session
    console.warn("Persistent storage unavailable. Data will be lost on refresh.");
  }
}

// ─── Utilities ─────────────────────────────────────────────────────────────────
function fmt(n: number): string { return `S$${Number(n).toFixed(2)}`; }
function getMon(date: string): string {
...
export default function App() {
  const [expenses, setExpensesState] = useState<Transaction[]>([]);
  const [storageReady, setStorageReady] = useState(false);
  const [storageStatus, setStorageStatus] = useState<"loading" | "persistent" | "local" | "memory">("loading");
  ...
  useEffect(() => {
    (async () => {
      ...
      // Detect which storage is actually working
      if (typeof window !== "undefined" && (window as { storage?: unknown }).storage) {
  ...
  const weekly = useMemo(() => {
    ...
    filtered.forEach(e => { const m = getMon(e.date); w[m] = (w[m] || 0) + e.price; });
    return Object.entries(w).sort(([a], [b]) => a.localeCompare(b)).map(([k, v]) => ({ label: `Wk ${k}`, value: v }));
  }, [filtered]);
  const monthly = useMemo(() => {
    const m: Record<string, number> = {};
    expenses.forEach(e => { const k = e.date.slice(0, 7); m[k] = (m[k] || 0) + e.price; });
  ...
  // ── Actions ───────────────────────────────────────────────────────────────────
  const openDrilldown = (cat: Category | "All") => {
    setDrilldown(cat);
    setTimeout(() => document.getElementById("drilldown-panel")?.scrollIntoView({ behavior: "smooth", block: "start" }), 50);
  };
  ...
  const submit = () => {
    ...
    setErr("");
    const newEntry = { ...form, price };
    if (editId !== null) {
      setExpenses(p => p.map(e => e.id === editId ? { ...e, ...newEntry } : e));
      setEditId(null);
    } else {
      setExpenses(p => [...p, { ...newEntry, id: Date.now() }]);
    }
    setForm(emptyForm);
    setOk(true);
  ...
  const storageBadge: Record<string, { label: string; color: string; bg: string }> = {
    loading:    { label: "⏳ Loading...",              color: "#555",   bg: "#f0f0f0" },
    persistent: { label: "☁️ Saved to Cloud (Persistent)", color: "#065F46", bg: "#D1FAE5" },
    local:      { label: "💾 Saved to Browser (localStorage)", color: "#1e3a8a", bg: "#DBEAFE" },
    memory:     { label: "⚠️ Memory Only (resets on refresh)", color: "#92400e", bg: "#FEF3C7" },
  };
  const badge = storageBadge[storageStatus];

  // ── Loading screen ─────────────────────────────────────────────────────────────
  if (!storageReady) {
    return (
      <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",maxWidth:900,margin:"0 auto",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f7f8fc"}}>
        <div style={{textAlign:"center",color:"#888"}}>
          <div style={{fontSize:32,marginBottom:12}}>💰</div>
          <div style={{fontSize:14,fontWeight:600}}>Loading your expenses...</div>
        </div>
      </div>
    );
  }
    ...
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",maxWidth:900,margin:"0 auto",background:"#f7f8fc",minHeight:"100vh",padding:16}}>
      ...
      <div style={{...card, background:"linear-gradient(135deg,#4F46E5,#7C3AED)", color:"#fff", marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontSize:22,fontWeight:800}}>💰 Couple Expense Tracker</div>
            <div style={{fontSize:13,opacity:.75,marginTop:2}}>Stay on top of your finances together</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:11,opacity:.65}}>All-time spending</div>
            <div style={{fontSize:22,fontWeight:800}}>{fmt(expenses.reduce((s, e) => s + e.price, 0))}</div>
          </div>
        </div>
      </div>

      {/* Storage status badge */}
      <div style={{...badge, borderRadius:10, padding:"8px 14px", marginBottom:12, fontSize:12, fontWeight:600}}>
        {badge.label}
      </div>
      ...
      <div style={{display:"flex",background:"#fff",borderRadius:12,padding:6,gap:4,marginBottom:16,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
        {[["dashboard","📊 Dashboard"],["add","➕ Add"],["records","📋 Records"],["insights","💡 Insights"]].map(([id, label]) => (
          <button key={id} onClick={() => { setTab(id); if (id !== "dashboard") setDrilldown(null); }}
            style={{flex:1,padding:"10px 6px",border:"none",borderRadius:8,cursor:"pointer",fontWeight:600,fontSize:13,background:tab===id?"#4F46E5":"transparent",color:tab===id?"#fff":"#777"}}>
            {label}
          </button>
        ))}
      </div>
        ...
        <div style={{...card, padding:"14px 20px"}}>
          <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
            <div><label style={lbl}>Month</label>
              <select style={inp} value={fMonth} onChange={e => { setFMonth(e.target.value); setDrilldown(null); }}>
                <option value="All">All Time</option>
                {allMonths.map(m => <option key={m} value={m}>{new Date(m+"-02").toLocaleString("default",{month:"long",year:"numeric"})}</option>)}
              </select>
            </div>
            <div><label style={lbl}>Category</label>
              <select style={inp} value={fCat} onChange={e => { setFCat(e.target.value as Category | "All"); setDrilldown(null); }}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {topByCat.filter(c => c.value > 0).map(c => (
              <div key={c.label} onClick={() => openDrilldown(c.label as Category)}
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:12,fontWeight:700,color:"#333"}}>{c.label}</div>
                  <div style={{fontSize:11,color:"#888",marginTop:1}}>{c.item}</div>
                </div>
      ...
      {tab === "add" && <div style={card}>
        ...
        {ok && <div style={{background:"#D1FAE5",color:"#065F46",borderRadius:10,padding:"10px 16px",marginBottom:14,fontSize:13,fontWeight:600}}>✅ Expense {editId !== null ? "updated" : "added"} successfully! Saved to storage.</div>}
      ...
      {tab === "records" && <div>
        <div style={{...card, padding:"14px 20px"}}>
          <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
            ...
            <div><label style={lbl}>Category</label>
              <select style={inp} value={fCat} onChange={e => setFCat(e.target.value as Category | "All")}>
                <option value="All">All</option>
                {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div><label style={lbl}>Sort</label>
      ...
      {tab === "insights" && <div>
        ...
        {view === "monthly" && <div style={card}>
          <div style={{fontSize:15,fontWeight:700,color:"#333",marginBottom:14}}>📅 Monthly Spending Trend</div>
          <ClickableBar data={monthly} onClickBar={(label) => setFMonth(allMonths.find(m => m.endsWith(label.split(" ")[0].toLowerCase())) || "All")} />
        </div>}
        {view === "weekly" && <div>
          <div style={{...card, padding:"14px 20px"}}>
            <label style={lbl}>Filter Month</label>
            <select style={inp} value={fMonth} onChange={e => setFMonth(e.target.value)}>
              <option value="All">All Time</option>
              {allMonths.map(m => <option key={m} value={m}>{new Date(m+"-02").toLocaleString("default",{month:"long",year:"numeric"})}</option>)}
            </select>
          ...
          <div style={card}>
            <div style={{fontSize:15,fontWeight:700,color:"#333",marginBottom:14}}>📆 Weekly Breakdown</div>
            <ClickableBar data={weekly} onClickBar={() => {}} />
          </div>
        </div>}
        <div style={card}>
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
              ...
              <tbody>
                {CATEGORIES.map(cat => {
                  ...
                  return (
                    <tr key={cat} style={{borderBottom:"1px solid #f0f0f0"}}>
                      ...
                      {allMonths.map(m => {
                        const v = expenses.filter(e => e.category === cat && e.date.startsWith(m)).reduce((s, e) => s + e.price, 0);
                        return <td key={m} style={{padding:"10px",textAlign:"right",color:v>0?"#333":"#ccc"}}>{v > 0 ? fmt(v) : "—"}</td>;
                      })}
                ...
                <tr style={{background:"#f7f8fc",borderTop:"2px solid #e0e0e0"}}>
                  ...
                  {allMonths.map(m => {
                    const v = expenses.filter(e => e.date.startsWith(m)).reduce((s, e) => s + e.price, 0);
                    return <td key={m} style={{padding:"10px",textAlign:"right",fontWeight:700}}>{v > 0 ? fmt(v) : "S$0.00"}</td>;
                  })}
